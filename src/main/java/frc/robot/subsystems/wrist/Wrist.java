...
public class Wrist extends BlitzSubsystem {
  ...
  //                     Commands.waitUntil(() -> {
  //                         boolean near = MathUtil.isNear(position, getPosition(), TOLERANCE);
  //                         System.out.println("position: " + getPosition() + " target: " +
  // position + " near: " + near);
  //                         return near;
  //                     }))
  ...
  //                     MathUtil.clamp(goal, Math.min(EXTENDED_POS, IDLE_POS),
  // Math.max(EXTENDED_POS, IDLE_POS)), 0));
  //         }
  //     })
  //             .handleInterrupt(() -> {
  //                 System.out.println("Interrupted, goal was: " + this.goal.map(s ->
  // String.valueOf(s.position)).orElse("empty"));
  ...
  //        return runOnce(() -> setpoint = new TrapezoidProfile.State(getPosition(),
  // getVelocity()))
